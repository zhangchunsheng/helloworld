...
//    }
//    
    ITTDINFO(@"is leap year %d", [CalendarDateUtil isLeapYear:2012]);
    ITTDINFO(@"current year %d", [CalendarDateUtil getCurrentYear]);
    ITTDINFO(@"numberOfDaysInMonth %d", [CalendarDateUtil numberOfDaysInMonth:4]);    
    ITTCalDay *calDay = [[ITTCalDay alloc] initWithDate:[NSDate dateWithTimeIntervalSinceNow:2*24*60*60]];
    ITTDINFO(@"cal day %@", calDay);
    [calDay release];
    NSInteger x = 12;    
    NSAssert(x!=0, @"x must not be zero");    
    ITTCalMonth *calMonth = [[ITTCalMonth alloc] initWithMonth:4 year:2012];
